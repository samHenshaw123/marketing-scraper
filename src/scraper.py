...
def scrape_national_tyre_data(national_url):
    parsed_url = urlparse(national_url)
    base_url = f"{parsed_url.scheme}://{parsed_url.netloc}"

    try: 
        response = requests.get(national_url, timeout=10)
...
def scrape_black_circles_tyre_data(black_circles_url):
    parsed_url = urlparse(black_circles_url)
    base_url = f"{parsed_url.scheme}://{parsed_url.netloc}"
    
    try:
        response = requests.get(black_circles_url, timeout=10)
        response.raise_for_status()
        time.sleep(2)
